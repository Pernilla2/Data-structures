...
M=(w*(-6*x**2+L*x-L**2))/12
V=w*(L/2-x)
a='The beneding moment at x=L is '
...
b=-L
c=L**2/6
#Using the Almight formula the two roots are; 
...
M=(w*(-6*x**2+L*x-L**2))/12
print()
print('(d)Using the intialized variable,the bending moment at each step in the array is{0}'.format(M))

#Question e
V=w*(L/2-x)
print()
print('(e)The shear force for each step alone the span is{}'.format(V))
...
#Question h
7#Let the maximum bending moment be max_M and the minimum bending moment be min_M
#for the maximum
max_M=max(M)
...
https://github.com/Pernilla2/Data-structures.git
